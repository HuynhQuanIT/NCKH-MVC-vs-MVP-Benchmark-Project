 ...
 * ============================================================
 *  Gộp: generate_images.php + seed + benchmark_step.php (cũ).
 *
 *  ĐÃ BỎ HOÀN TOÀN 12 sản phẩm thật cố định (REAL_PRODUCTS) —
 *  bảng products giờ CHỈ chứa đúng N sản phẩm mock bạn chỉ định,
 *  số liệu luôn là số chẵn tròn (1.000 / 5.000 / 50.000 / 500.000),
 *  không còn lẻ "+12" như bản trước.
 *
 *  Tự động:
 *   1. Sinh ảnh SVG theo loại (CHỈ 1 LẦN, bỏ qua nếu đã có)
 *   2. Re-seed bảng products đúng N sản phẩm mock (TRUNCATE + insert)
...
// ════════════════════════════════════════════════════════════
// 1. LOẠI SẢN PHẨM + ẢNH — chỉ sinh nếu còn THIẾU
// ════════════════════════════════════════════════════════════
...
function ensure_images(mysqli $conn): void {
...
// ════════════════════════════════════════════════════════════
// 2. SEED — TRUNCATE rồi insert ĐÚNG $mockTotal dòng, không +12
// ════════════════════════════════════════════════════════════
function seed_to_total(mysqli $conn, int $mockTotal, array $typeIds, int $batch = 2000): array {
    ...
    $max = $conn->query("SELECT MAX(idProduct) AS m FROM products")->fetch_assoc()['m'] ?? 0;
    ...
    return ['mock' => $inserted, 'total' => (int)$total, 'seed_ms' => $seedMs];
}
...
function run_one_scale(mysqli $conn, array $typeIds, int $mockTotal, int $iters): array {
    ...
    $total = $stats['total'];
    echo "Seed xong: $total sản phẩm mock ({$stats['seed_ms']} ms)\n";
...
if ($runAll) {
    ...
    $mockTotal = isset($_GET['mock']) ? max(1, (int)$_GET['mock']) : 1000;
...
echo "<hr><p>DB hiện đang giữ <b>$lastTotal sản phẩm</b> (mức mock=$lastMock) — mở 
